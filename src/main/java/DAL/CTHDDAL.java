...
public class CTHDDAL {
    ...
    public Boolean setUpCTHD(String maTour, String lydohuy, Boolean hoanTien) throws DaoException {
        String sql = "{call HuyHoaDonDatTour(?,?,?)}";
        try (Connection con = conn.getConnection();
                CallableStatement call = con.prepareCall(sql);) {
            call.setString(1, maTour);
            call.setString(2, lydohuy);
            call.setBoolean(3, hoanTien);
