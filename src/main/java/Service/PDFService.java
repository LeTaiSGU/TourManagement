...
public class PDFService {
    ...
    public static String createHoaDonHoanTienHuyTourPDF(HoaDon hd, ArrayList<CTHD> dsct, String tienhoan,
            String lydohuy) {
        String path = "HoaDonPDF/HOAN_TIEN_HUY_TOUR/" + hd.getMaHoaDon() + ".pdf";

        try {
            File file = new File(path);
            if (!file.exists())
                taoHoaDonHoanTienHuyTourPDF(hd, dsct, path, tienhoan, lydohuy);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return path;
    ...
    private static void taoPDF(HoaDon hd, ArrayList<CTHD> dsct, String path, boolean daThanhToan) {
        try {
            ...
            if (!daThanhToan) {
                ...
                document.add(
                        new Paragraph("NGÀY ĐẾN HẠN THANH TOÁN: " + han.format(dtf2)).setFont(boldFont)
                                .setFontColor(red));
            } else {
                document.add(new Paragraph("Đã thanh toán: " + df.format(total)).setFont(boldFont).setFontSize(14)
                        .setFontColor(red));
                document.add(new Paragraph("Thời gian thanh toán: " + dtf.format(LocalDateTime.now())).setFont(boldFont)
                        .setFontSize(14).setFontColor(red));
            }
    ...
    private static void taoHoaDonHoanTienPDF(HoaDon hd, ArrayList<CTHD> dsct, String path, String tienhoan,
            String lydohuy) {
        try {
            ...
            document.add(
                    new Paragraph("Số tiền hoàn trả: " + tienhoan).setFont(boldFont).setFontSize(14).setFontColor(red));
            document.add(new Paragraph("Thời gian hoàn tiền: " + dtf.format(LocalDateTime.now())).setFont(boldFont)
                    .setFontSize(14));
    ...
    private static void taoHoaDonHoanTienHuyTourPDF(HoaDon hd, ArrayList<CTHD> dsct, String path, String tienhoan,
            String lydohuy) {
        try {
            boolean thanhtoan = hd.isTrangThaiTT();
            PdfWriter writer = new PdfWriter(path);
